...
def test_reference_within_threshold_is_used_without_a_warning() -> None:
    feature_id = uuid.uuid4()

    resolved = resolve_feature_area(feature_id, SQUARE_100M, 10_200.0, crs=31982)

    assert resolved.area_m2 == pytest.approx(10_200.0)
    assert resolved.warning is None


def test_reference_diverging_past_the_threshold_is_used_but_warns() -> None:
    feature_id = uuid.uuid4()

    resolved = resolve_feature_area(feature_id, SQUARE_100M, 8_000.0, crs=31982)

    assert resolved.area_m2 == pytest.approx(8_000.0)
